...
export function SpeedTestHistory({ athleteId, refreshKey, onDeleted }: Props) {
  ...
  const handleDelete = async (testId: number) => {
    ...
    if (
      !window.confirm(
        "¿Eliminar este test de velocidad? Esta acción no se puede deshacer.",
      )
    ) {
      return;
    }
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 bg-slate-50">
        ...
        <p className="mt-1 text-sm text-slate-600">Listado de todos los tests MSS registrados para este atleta.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-6">
          <EmptyStateCard
            ...
            description="Cargá el primero desde el formulario de Test de Velocidad."
          />
        </div>
      ) : (
